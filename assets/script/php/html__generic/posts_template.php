<div class="post_clone type_1" style="display:none;">
    <div class="post--menu--area">
        <div class="info-compartilhador" style="display: none;">
            Repostado por <a href='' class="user_respost">
                <div class="event min-event"></div>
            </a>
        </div>
        <div class="header--post--area">
            <div class="post--area--perfil">
                <div class="img--perfil menu--pag--img--area">
                </div>
                <div class="name--area">
                    <a class="a_nav perfil-link" href="">
                        <div class="name--name-perfil perfil-link-hover">
                        </div>
                        <div class="name--username-perfil perfil-link-hover">

                        </div>
                    </a>
                </div>
            </div>
            <div class="post--area--game">
                <div class="game--post">
                    
                </div>
            </div>
            <div class="post--area--date ">
                <div class="date--post">
                </div>
            </div>

            <div class="post--area--menu ">
                <a href="" class="a_nav elipse-img-hover elipse-img" aria-label="Menu"></a>

            </div>

            <!--deve ter o nome e @ do usuario e o menu de denuncia de cada usuario-->
        </div>
        <div class="body--post--area">
            <div class="post--text"></div>
            <div class="post--img-area" style="display: none;">
                <div class="post--img">
                    <div class="event--post--img"></div>
                </div>
            </div>
            <!--deve ter oq o usuario publicou-->
        </div>
        <a class="conteudo--all--post" href="">
            mostrar todo conteúdo
        </a>
        <div class="interacao--post--area">
            <form class="event--curtida curtir--hover interac-button">
                <input type="hidden" value="" name="p-xD30">
                <button class="curtir">
                    Curtir
                </button>
                <div class="post_curtidas area_num">
                    <div class="event min-event"></div>
                </div>
            </form>
            <div class="comentar post_coment_area">
                <a href="" class="a_nav">
                    <div class=" img--small img--iteracao interacao--area">
                        comentar
                    </div>
                </a>
                <div class="post_comentadas area_num">5</div>
            </div>
            <div class=" compartilhar-hover compartilhar-event-div interac-button  interac-button ">
                <button class=" compartilhar-event img--iteracao img--strong button--remove interacao--area ">
                    compartilhar
                </button>
                <div class="post_compartilhadas area_num">
                    <div class="event min-event"></div>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="post_clone type_2" style="display:none;">
    <div class="post--menu--area">
        <div class="header--post--area">
            <div class="post--area--perfil">
                <div class="img--perfil menu--pag--img--area">
                </div>
                <div class="name--area name--comp">
                    <a class="a_nav perfil-link" href="">
                        <div class="name--name-perfil name--name-perfil-comp_ perfil-link-hover">
                        </div>
                        <div class="name--username-perfil name--username-perfil-comp_ perfil-link-hover">
                        </div>
                    </a>
                </div>
            </div>

            <div class="post--area--game">
                <div class="game--post">
                 
                </div>
            </div>

            <div class="post--area--date">
                <div class="date--post date--post-comp_">
                </div>
            </div>

            <div class="post--area--menu ">
                <a href="" class="a_nav elipse-img-hover elipse-img" aria-label="Menu"></a>
            </div>

            <!--deve ter o nome e @ do usuario e o menu de denuncia de cada usuario-->
        </div>
        <div class="body--post--area">
            <div class="post--text_comp post--text"></div>
            <div class="post--img-area" style="display: none; margin-bottom: 5px;">
                <div class="post--img">
                    <div class="event--post--img"></div>
                </div>
            </div>
            <div class="area--post-com" style="display: block;">
                <div class="post--comp">
                    <div class="header--post--area">
                        <div class="post--area--perfil">
                            <div class="img--perfil-comp menu--pag--img--area">
                            </div>
                            <div class="name--area">
                                <a class="a_nav perfil-link perfil-link-comp" href="">
                                    <div class="name--name-perfil-comp perfil-link-hover">

                                    </div>
                                    <div class="name--username-perfil-comp perfil-link-hover">

                                    </div>
                                </a>
                            </div>
                        </div>

                        <div class="post--area--game">
                            <div class="game--post-comp">
                                
                            </div>
                        </div>

                        <div class="post--area--date">
                            <div class="date--post-comp ">

                            </div>
                        </div>

                        <div class="post--area--menu ">
                            <a href="" class="a_nav elipse-img-hover elipse-img" aria-label="Menu"></a>
                        </div>

                        <!--deve ter o nome e @ do usuario e o menu de denuncia de cada usuario-->
                    </div>
                    <div class="body--post--area">
                        <div class="post--text post--text--comp_2"></div>
                        <div class="post--img-area post--img-area-com" style="display:none">
                            <div class="post--img img--comentada">
                                <div class="event--post--img"></div>
                            </div>
                        </div>
                        <!--deve ter oq o usuario publicou-->
                    </div>
                </div>
            </div>
            <!--deve ter oq o usuario publicou-->
        </div>
        <a class="conteudo--all--post" href="">
            mostrar todo conteudo
        </a>
        <div class="interacao--post--area">
            <form class="event--curtida event--curtida-comp curtir--hover interac-button">
                <input type="hidden" value="" name="p-xD30">
                <button class="curtir interacao--area button--remove img--iteracao img--iteracao-curtida p-evt-box-off">
                    Curtir
                </button>
                <div class="post_curtidas area_num">
                    <div class="event min-event"></div>
                </div>
            </form>
            <div class="comentar post_coment_area">
                <a href="" class="a_nav">
                    <div class="img--small img--iteracao interacao--area">
                        comentar
                    </div>
                </a>
                <div class="post_comentadas area_num">5</div>
            </div>
            <div class="compartilhar-hover compartilhar-event-div interac-button  interac-button">
                <button class="compartilhar-event img--iteracao img--strong button--remove interacao--area">
                    compartilhar
                </button>
                <div class="post_compartilhadas area_num">
                    <div class="event min-event"></div>
                </div>
            </div>
        </div>
    </div>
</div>